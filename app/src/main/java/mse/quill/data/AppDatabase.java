...
public class AppDatabase extends SQLiteOpenHelper {
    ...
    /**
     * Never destructive, on any path.
     *
     * <p>Every upgrade runs the same idempotent, column-driven {@link #ensureAdditiveSchema}. The
     * only thing the version number decides is whether the pre-Markdown note format has to be
     * converted first — and that conversion reads the old rows rather than dropping them.
     *
     * <p>This used to drop and recreate everything below v3. That was accepted as dev-stage policy
     * and duly wiped a real notebook on 2026-07-28. A version bump must not be able to do that
     * again, so the rebuild path is <em>deleted</em> rather than merely unreachable: there is no
     * branch left for a later edit to fall back into.
     */
    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion < 3) migrateLegacyNotesToMarkdown(db);
    ...
    /**
     * Converts a pre-v3 database in place: the note bodies first, then the table that held them.
     *
     * <p>Before v3 a note's body was <em>rows</em> — one {@code note_segments} row per block,
     * ordered by a {@code position} column, with formatted text stored in {@code text_content} as
     * HTML (that is what the since-deleted {@code SpanSerializer} wrote: {@code Html.toHtml}, not a
     * custom binary format, which is the reason this migration is possible at all). From v3 the
     * body is one Markdown document in {@code notes.content_blob}, and {@code note_segments} keeps
     * only what a Markdown link has nowhere to put — the media registry.
     *
     * <p>The reshape is not optional housekeeping. The old table declares
     * {@code position INTEGER NOT NULL}, and no code since v3 supplies a position, so leaving the
     * old shape in place would fail every insert of an image or a recording with a NOT NULL
     * constraint violation — a database that upgraded but could no longer be written to.
     *
     * <p>Media row ids are carried across unchanged, because {@link NoteDocument#toMarkdown} writes
     * them into the document as {@code quill://image/<id>}. A regenerated id would be a document
     * pointing at a registry row that no longer answers to that name, which reads to the user as
     * every picture in every old note disappearing at once.
     */
    private void migrateLegacyNotesToMarkdown(SQLiteDatabase db) {
    ...
    private String legacyNoteToMarkdown(SQLiteDatabase db, String noteId) {
     ...
     */
    private android.text.Spannable legacyHtmlToSpannable(byte[] bytes) {
     ...
     */
    private void reshapeLegacySegmentTable(SQLiteDatabase db) {
