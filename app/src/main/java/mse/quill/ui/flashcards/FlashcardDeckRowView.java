...
/**
 * One deck row: a due-count badge, the note's title with two lines of detail, and a delete button.
 *
 * <p>Built in code rather than inflated, for the reason documented on {@code ui.home.NoteRowView} —
 * on this SDK the first XML-derived LayoutParams inside an {@code inflate()} call throws. Styling
 * comes from {@link CardStyles} so a deck row and a note row are visibly the same kind of object.
 *
 * <p>The badge leads because "how many reviews are left" is the one number worth scanning down a
 * list for; the counts and the schedule follow underneath as detail.
 */
final class FlashcardDeckRowView {

    private FlashcardDeckRowView() {}
    ...
    static Views build(Context context) {
        ...
        badge.setMinWidth(CardStyles.dimen(context, R.dimen.deck_badge_min_width));
        ...
        // A visible control rather than a long-press: deleting a deck is the only destructive thing
        // on this screen, and a hidden gesture for it would be either undiscoverable or, once
        // discovered, easy to trigger by accident while scrolling.
        ...
        delete.setContentDescription(context.getString(R.string.action_delete_flashcards));
